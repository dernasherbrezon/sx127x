...
TEST_CASE("sx127x_test_lora_rx_deepsleep", "[lora]") {
    ...
    TEST_ASSERT_EQUAL_INT(SX127X_OK,rtc_gpio_set_direction((gpio_num_t)fixture_config.dio0, RTC_GPIO_MODE_INPUT_ONLY));
    TEST_ASSERT_EQUAL_INT(SX127X_OK,rtc_gpio_pulldown_en((gpio_num_t)fixture_config.dio0));
    ESP_LOGI("sx127x_test", "entering deep sleep");
    TEST_ASSERT_EQUAL_INT(SX127X_OK,esp_sleep_enable_ext0_wakeup((gpio_num_t)fixture_config.dio0, 1));
...
TEST_CASE("sx127x_test_lora_rx_implicit_header", "[lora]") {
    ESP_ERROR_CHECK(sx127x_lora_set_implicit_header(&explicit_header, fixture->device));
    sx127x_rx_set_callback(rx_callback, fixture->device);
    TEST_ASSERT_EQUAL_INT(SX127X_OK, sx127x_set_opmod(SX127x_MODE_RX_CONT, SX127x_MODULATION_LORA, fixture->device));
    xSemaphoreTake(fixture->rx_done, TIMEOUT);
    uint8_t expected[] = {0xCA, 0xFE};
...
void setUp() {
    if( esp_sleep_get_wakeup_cause() !=  ESP_SLEEP_WAKEUP_EXT0) {
        TEST_ASSERT_EQUAL_INT(SX127X_OK, sx127x_fixture_create(&fixture_config, &fixture));
    } else {
        TEST_ASSERT_EQUAL_INT(SX127X_OK, sx127x_fixture_create_base(&fixture_config, &fixture));
    }
}
