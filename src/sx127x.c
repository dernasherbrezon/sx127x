...
int sx127x_append_register(int reg, uint8_t value, uint8_t mask, sx127x *device) {
  uint8_t previous = 0;
  int code = sx127x_read_register(reg, device, &previous);
  if (code != SX127X_OK) {
    return code;
  }
  uint8_t data[] = {(previous & mask) | value};
  return sx127x_spi_write_register(reg, data, 1, device);
}
...
void sx127x_handle_interrupt(sx127x *device) {
  ...
  // clear the irq
  uint8_t data[] = {value};
  code = sx127x_spi_write_register(REG_IRQ_FLAGS, data, 1, device);
  if (code != SX127X_OK) {
    return;
  }
...
int sx127x_create(void *spi_device, sx127x **result) {
  ...
  *device = (struct sx127x_t){0};
  ...
  int code = sx127x_read_register(REG_VERSION, device, &device->version);
  if (code != SX127X_OK) {
...
int sx127x_set_opmod(sx127x_mode_t opmod, sx127x *device) {
  ...
  return sx127x_spi_write_register(REG_OP_MODE, data, 1, device);
}
...
int sx127x_set_frequency(uint64_t frequency, sx127x *device) {
  ...
  int result = sx127x_spi_write_register(REG_FRF_MSB, data, 3, device);
  if (result != SX127X_OK) {
    return result;
  }
...
int sx127x_reset_fifo(sx127x *device) {
  ...
  return sx127x_spi_write_register(REG_FIFO_TX_BASE_ADDR, data, 2, device);
}
...
int sx127x_set_bandwidth(sx127x_bw_t bandwidth, sx127x *device) {
  int code = sx127x_append_register(REG_MODEM_CONFIG_1, bandwidth, 0b00001111, device);
  if (code != SX127X_OK) {
    return code;
  }
...
int sx127x_set_modem_config_2(sx127x_sf_t spreading_factor, sx127x *device) {
  ...
  int code = sx127x_spi_write_register(REG_DETECTION_OPTIMIZE, data, 1, device);
  if (code != SX127X_OK) {
    return code;
  }
  data[0] = detection_threshold;
  code = sx127x_spi_write_register(REG_DETECTION_THRESHOLD, data, 1, device);
  if (code != SX127X_OK) {
    return code;
  }
...
int sx127x_set_syncword(uint8_t value, sx127x *device) {
  uint8_t data[] = {value};
  return sx127x_spi_write_register(REG_SYNC_WORD, data, 1, device);
}

int sx127x_set_preamble_length(uint16_t value, sx127x *device) {
  uint8_t data[] = {(uint8_t)(value >> 8), (uint8_t)(value >> 0)};
  return sx127x_spi_write_register(REG_PREAMBLE_MSB, data, 2, device);
}
...
int sx127x_set_implicit_header(sx127x_implicit_header_t *header, sx127x *device) {
  ...
  if (header == NULL) {
    ...
    int code = sx127x_append_register(REG_MODEM_CONFIG_1, SX127x_HEADER_MODE_IMPLICIT | device->header->coding_rate, 0b11110000, device);
    if (code != SX127X_OK) {
      return code;
    }
...
int sx127x_read_payload(sx127x *device, uint8_t **packet, uint8_t *packet_length) {
  ...
  if (device->header == NULL) {
    int code = sx127x_read_register(REG_RX_NB_BYTES, device, &length);
    if (code != SX127X_OK) {
      *packet_length = 0;
      *packet = NULL;
      return code;
  ...
  uint8_t current;
  int code = sx127x_read_register(REG_FIFO_RX_CURRENT_ADDR, device, &current);
  if (code != SX127X_OK) {
    *packet_length = 0;
    *packet = NULL;
    return code;
  ...
  code = sx127x_spi_write_register(REG_FIFO_ADDR_PTR, data, 1, device);
  if (code != SX127X_OK) {
    *packet_length = 0;
    *packet = NULL;
    return code;
...
int sx127x_get_packet_rssi(sx127x *device, int16_t *rssi) {
  uint8_t value;
  int code = sx127x_read_register(REG_PKT_RSSI_VALUE, device, &value);
  if (code != SX127X_OK) {
    return code;
  }
...
int sx127x_get_packet_snr(sx127x *device, float *snr) {
  uint8_t value;
  int code = sx127x_read_register(REG_PKT_SNR_VALUE, device, &value);
  if (code != SX127X_OK) {
    return code;
  }
...
int sx127x_get_frequency_error(sx127x *device, int32_t *result) {
  ...
  int code = sx127x_spi_read_registers(REG_FREQ_ERROR_MSB, device, 3, &frequency_error);
  if (code != SX127X_OK) {
    return code;
  }
  uint32_t bandwidth;
...
int sx127x_set_dio_mapping1(sx127x_dio_mapping1_t value, sx127x *device) {
  uint8_t data[] = {value};
  return sx127x_spi_write_register(REG_DIO_MAPPING_1, data, 1, device);
}

int sx127x_set_dio_mapping2(sx127x_dio_mapping2_t value, sx127x *device) {
  uint8_t data[] = {value};
  return sx127x_spi_write_register(REG_DIO_MAPPING_2, data, 1, device);
}
...
int sx127x_set_pa_config(sx127x_pa_pin_t pin, int power, sx127x *device) {
  ...
  int code = sx127x_spi_write_register(REG_PA_DAC, data, 1, device);
  if (code != SX127X_OK) {
    return code;
  }
  uint8_t max_current;
  ...
  data[0] = value;
  return sx127x_spi_write_register(REG_PA_CONFIG, data, 1, device);
}
...
int sx127x_set_ocp(sx127x_ocp_t onoff, uint8_t max_current, sx127x *device) {
  ...
  if (onoff == SX127x_OCP_OFF) {
    ...
    return sx127x_spi_write_register(REG_OCP, data, 1, device);
  }
...
int sx127x_set_tx_explicit_header(sx127x_tx_header_t *header, sx127x *device) {
  ...
  int code = sx127x_append_register(REG_MODEM_CONFIG_1, header->coding_rate | SX127x_HEADER_MODE_EXPLICIT, 0b11110000, device);
  if (code != SX127X_OK) {
    return code;
  }
  return sx127x_append_register(REG_MODEM_CONFIG_2, header->crc, 0b11111011, device);
...
int sx127x_set_for_transmission(uint8_t *data, uint8_t data_length, sx127x *device) {
  ...
  int code = sx127x_spi_write_register(REG_FIFO_ADDR_PTR, fifo_addr, 1, device);
  if (code != SX127X_OK) {
    return code;
  }
  uint8_t reg_data[] = {data_length};
  code = sx127x_spi_write_register(REG_PAYLOAD_LENGTH, reg_data, 1, device);
  if (code != SX127X_OK) {
    return code;
  }
